...
setInterval(() => io.emit(state), 1000)

setInterval(() => printInterface(), 100)
...
app.get('/interface', (req, res) => require('./send')(req, res))

app.post("/message", async function (request, response) {
    let d = (await manager.process('en', message)).classifications.filter(a=>a.score > 0.02 ).map(a=>a.intent)
    updateState(d)
